...
//creating a middleWare to check if the input is a number or not and not left blank
app.use(function(req, res, next) {
  ...
  if (num1 != "" && num2 != "") {
    if (false === isNaN(num1) && false === isNaN(num2)) {
      ...
      res.send(
        "<h1 style='color:blue ;background-color:grey'> Please Enter a valid number</h1>"
      );
    }
  } else {
    res.send(
